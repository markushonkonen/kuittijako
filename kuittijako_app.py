...
# --- Asetukset ---
IGNORE_WORDS = [
    "NORM.", "KAMPANJA", "RIVIALENNUS", "BONUSTA", "ALV",
    "PUH", "WWW.", "AVOINNA", "K-CITYMARKET", "KANTA-",
    "KORTTI", "VISA", "YHTEENSÄ", "VERO", "DEBIT", "VEL",
    "PALVELU", "KIITOS", "KÄYNTI", "OYJ", "ASIAKAS", "KÄTEINEN",
    "MAKSUKORTTI", "KASSA", "TAPAHTUMAT", "PLUSSAA", "OSTOT",
    "TAVARAT", "TILI", "KÄYTTÖTAVARAA"
]
...
if uploaded_file:
    ...
    for line in lines:
        ...
        # Ohitetaan rivit, jotka sisältävät selvästi ei-tuotetietoa
        if any(word in clean_line for word in IGNORE_WORDS):
            continue

        # Ohitetaan liian lyhyet tai tyhjät rivit
        if len(clean_line) < 5:
            continue

        # Yritä pilkkoa rivi muotoon: "tuotenimi hinta"
        parts = line.rsplit(" ", 1)
        if len(parts) == 2:
            name, price = parts

            # Jos rivin loppu näyttää puhelinnumerolta, ohitetaan
            if re.search(r"\d{4,}", name):  # esim. "0105382200" tai "3014"
                continue
