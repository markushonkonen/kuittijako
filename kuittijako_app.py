...
# --- Asetukset ---
IGNORE_WORDS = ["NORM.", "KAMPANJA", "RIVIALENNUS", "BONUSTA", "ALV", "PUH.", "VIITE"]
...
if uploaded_file:
    ...
    for line in lines:
        ...
        # Lopetetaan jos "YHTEENSÄ"-rivi löytyy
        if re.search(r"\bYHTEENSÄ\b", clean_line):
            match = re.search(r"YHTEENSÄ\s+([\d,\.]+)", line.upper())
            if match:
                try:
                    total_sum = float(match.group(1).replace(",", "."))
                except ValueError:
                    pass
            break

        # Ohita selvästi ei-tuoterivit
        ...
        # Pilkotaan viimeinen osa hinnaksi
        parts = line.rsplit(" ", 1)
        if len(parts) == 2:
            name, price = parts

            # Ohita selvästi väärät rivit, joissa nimi on liian numero-painotteinen
            if re.search(r"\d{5,}", name):  # esim. viitteet, tuotekoodit
                continue

            try:
                price_val = float(price.replace(",", "."))
                if price_val > 0 and len(name.strip()) > 1:
    ...
    if products:
        ...
        if total_sum:
            st.info(f"Kuitin loppusumma: **{total_sum:.2f} €**")
        else:
            st.warning("Kuitin loppusummaa ei voitu lukea.")
        ...
        for idx, (name, price) in enumerate(products):
            ...
            with col2:
                remove_flags[(name, price)] = st.checkbox("Poista", key=f"remove_{idx}")
        ...
        if st.button("Laske jako"):
            ...
            for (name, price), choice in choices.items():
                if remove_flags.get((name, price)):
                    continue  # käyttäjä poistanut rivin
